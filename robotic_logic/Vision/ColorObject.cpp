    ...
    if(contours.size() > 0){
        if(this->color == params::black){
            ...
            for(int i =0 ; i < contours.size() ; i++){
                ...
                if(area > (*objectSizeMin/2)*50 && area < (*objectSizeMax/2)*50){
            ...
            if(maxContourArea > (*objectSizeMin/2)*50 && maxContourArea < (*objectSizeMax/2)*50){
            ...
            for(int i =0 ; i < contours.size() ; i++){
                ...
                if(area > (*objectSizeMin/2)*50 && area < (*objectSizeMax/2)*50){
                    ...
                    //cout << "arcLength is :" << arcLen << endl;
                    approxPolyDP(contours[i],shapePoints,(*shapeDetail/200.0)*arcLen,true);
                    // drawContours(paintingFrame, vector<vector<Point> >(1,shapePoints),-1,getColorRGB(color));
                    drawPoints(paintingFrame, shapePoints, getColorRGB(color));
                    ...
                    Moments shapeMomments = moments(contours[i]);
                    Point centerPoint (shapeMomments.m10/shapeMomments.m00 , shapeMomments.m01/shapeMomments.m00);
                    cv::circle(paintingFrame, centerPoint, 2,getColorRGB(color),2);
                    ...
                    currObject.color = this->getColor();
                    if(shapePoints.size() == 3){
                        currObject.shape = params::triangle ;
                    }else if(shapePoints.size() == 4){
                        currObject.shape = params::rectangle ;
                    }else if(shapePoints.size() == 5){
                        currObject.shape = params::polygon ;
                    }else if(shapePoints.size() > 5){
                        currObject.shape = params::circle ;
                    }else{
                        continue;
                    }
                    currObject.update(geometry::Vector(0,0), pointsToGeometryVector(shapePoints));
...
vector<Point> getImportantPoints(vector<Point>& points){
    ...
    angles.push_back(make_pair(angle3(points[4], points[0], points[1]),0));
    angles.push_back(make_pair(angle3(points[0], points[1], points[2]),1));
    angles.push_back(make_pair(angle3(points[1], points[2], points[3]),2));
    angles.push_back(make_pair(angle3(points[2], points[3], points[4]),3));
    angles.push_back(make_pair(angle3(points[3], points[4], points[0]),4));
    sort(angles.begin(), angles.end(), [](const std::pair<double,int> &left, const std::pair<double,int> &right) {
        return left.first < right.first;
    });
    cout << " be tartib : " << angles[0].first << " "<< angles[1].first <<" "<< angles[2].first <<" "<< angles[3].first <<" "<< angles[4].first <<endl ;
    vector<Point> result;
    result.push_back(points[angles[4].second]);
    result.push_back(points[angles[3].second]);
    result.push_back(points[angles[2].second]);
    return result ;
}
